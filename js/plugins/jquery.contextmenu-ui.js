...
(function($) {
  ...
  $.fn.contextMenu = function(id, options) {
    ...
    hash.push({
      id: id,
      enableLeftClick: options.enableLeftClick || defaults.enableLeftClick,
  ...
  function display(index, trigger, e) {
    ...
    if(cur.menuPosition=='right')
    {
        menu.css({
            left: e[cur.eventPosX] - menu.width(),
            top: e[cur.eventPosY]
        }).show().removeAttr("aria-hidden");
        shadow_offset_x = menu.width();
    }
    else
    {
        menu.css({
    ...
    if (cur.shadow) {
      shadow.css({
        width: menu.width(),
        height: menu.height(),
        left: e.pageX + 2 - shadow_offset_x,
        top: e.pageY + 2
      }).show().removeAttr("aria-hidden");
    }
