...
tinymce.PluginManager.requireLangPack( 'magicline', 'es,ca,fr_FR,it,ro,ru' );
...
tinymce.PluginManager.add( 'magicline', function ( editor, url )
{
	...
	editor.on( 'init', function ()
	{
		...
		function checkMouse( E )
		{
			...
			while ( target.length > 0 )
			{
				...
				if ( 	!target.hasClass( 'nomagicline' ) &&
						targetedItems.indexOf( target[ 0 ].tagName ) > -1 &&
						!target.hasClass( 'mce-resize-bar' ) &&
						!target.hasClass( 'mce-resizehandle' )
				)
				{
					foundBlock = true;
					lastTarget = target;
			...
			DOM.bind( magicline, 'click',
			          function ( E )
			          {
				          if ( E.target === btInsert )
				          {
					          ...
					          if($(lastTarget).is("tr") || $(lastTarget).is("td"))
					          	lastTarget = $(lastTarget).closest('table');

					          if ( lastTargetPos === 'top' )
							  {
							  		lastTarget.before( p );
							  		p_inserted = $(lastTarget).prev();
                              }
					          else
							  {
							  		lastTarget.after( p );
                                  	p_inserted = $(lastTarget).next();
                              }

					          editor.selection.select( $(p_inserted)[0], true );
				          }
    ...
    function toggleMagicLine(new_state)
	{
    	...
    	editor.fire('magicline', {state: state});
    }

    function toggleActiveState()
	{
    	var self = this;
    	editor.on('magicline', function(e)
		{
    		self.active(e.state);
        });
    }

	editor.addCommand('mceMagicLine', toggleMagicLine);

	editor.addButton('magicline', {
		image : url + '/img/button.png',
		title: tinymce.util.I18n.translate( 'Toggle magic line' ),
		cmd: 'mceMagicLine',
