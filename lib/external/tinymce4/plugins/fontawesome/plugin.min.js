...
// modified by Marc Lobato from the Navigate CMS team
// use <i> instead of <span>
// allow modifiying a selected icon
// keep current tag attributes and additional classes
// added languages Catalan (translated) and Polish (placeholder)
...
tinymce.PluginManager.add('fontawesome', function(editor, url) {
    ...
    function showDialog() {
        ...
        function groupHtml(group, iconTitle) {
            ...
            gridHtml = '<div class="mce-fontawesome-panel-accordion">';
                gridHtml += '<div class="mce-fontawesome-panel-title">' + iconTitle + '<i class="mce-fontawesome-panel-accordion-indicator fa-chevron-right"></i></div>';
                    ...
                    for (var x = 0; x < width; x++) {
                        if (iconGroup[y * width + x]) {
                            ...
                            gridHtml += '<div class="mce-icon-cell js-mce-fontawesome-insert" title="' + name + '" data-id="' + id + '"' + (group === 'spinnerIcons' ? ' data-spin="true"' : '' ) + '>';
                                gridHtml += '<i class="fa fa-' + id + (group === 'spinnerIcons' ? ' fa-spin' : '' ) +'"></i>';
                            gridHtml += '</div>';
                        }
                    }
                }
        ...
        function insertIcon()
        {
            ...
            if($(selection_content).is("i"))
            {
                var existing_classes = $(selection_content)[0].classList;

                for(eci = 0; eci < existing_classes.length; eci++)
                {
                    found = false;
                    if(existing_classes[eci] == "fa")
                        continue;

                    // find if this class is a code for a font awesome icon
                    for(il=0; il < iconList.length; il++)
                    {
                        found = (existing_classes[eci] == "fa-" + iconList[il].id);
                        if(found)
                            break;
                    }

                    if(found)   // ignore this class
                        continue;

                    // class name not found in the icons list, include it!
                    new_classes = new_classes + " " + existing_classes[eci];
                }
                ...
                if($(editor.selection.getContent({format: 'raw'}))[0].hasAttributes())
                {
                    ...
                    {
                        if(or_attrs[i].name!='class')
                            new_attributes += ' ' + or_attrs[i].name + '="' + or_attrs[i].value + '" ';
                    }
                }
            }

            var content = '<i class="fa fa-' + id + '' + new_classes + '"' + new_attributes + '></i>';
    ...
    {
        // process commands to fontawesome <i> elements, if selected

        // get selected node, ignore if it's not an <i>
    ...
    */
    ...
    // the following two functions makes possible to replace an existing icon in the editor
    ...
    editor.addMenuItem('fontawesome', {
        icon: 'flag',
        text: "Font Awesome", //translate('Icons'),
        onclick: showDialog,
        context: 'insert'
    });
});
