...
function navigate_shipping_methods_onload(onload_language)
{
    ...
    navigate_shipping_methods_all_categories_switch();
}
...
function navigate_shipping_methods_all_categories_switch()
{
    $("#category-tree-parent").parent().hide();
    $("#category-exclusions-tree-parent").parent().hide();

    if($("#all_categories_2").is(":checked"))
    {
        $("#category-exclusions-tree-parent").parent().show();
    }
...
$("#all_categories_0,#all_categories_1,#all_categories_2").on("click", navigate_shipping_methods_all_categories_switch);

$("#category-tree-parent .tree_ul")
    .jstree({
...
$("#category-exclusions-tree-parent .tree_ul")
    .jstree({
        plugins: ["changed", "types", "checkbox"],
        "types" :
        {
    ...
    .on("dblclick.jstree", function(e)
    {
        ...
        var li = $(e.target).closest("li");
        $("#category-exclusions-tree-parent .tree_ul").jstree("open_node", "#" + li[0].id);
        ...
        $("#category-exclusions-tree-parent .tree_ul").jstree("select_node", children_nodes);
    ...
    .on("changed.jstree", function(e, data)
    {
        ...
        if(categories.length > 0)
        {
            $("#categories_exclusions").val(categories);
        }
    });
