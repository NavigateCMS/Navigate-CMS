...
{	
	...
	if($(el).attr("id")=="products-gallery-item-empty-droppable")
	{
		...
		new_node = '<li>' +
				   '<div id="products-gallery-item-' + file_id + '-droppable" website_id="' + navigate_media_browser_website + '" data-file-id="' + file_id + '" class="navigate-droppable ui-corner-all" data-media="image">' +
				   final_html +
				   '</div> ' +
...
{
	var price = parseFloat(navigatecms.forms.imask["product-base_price"].unmaskedValue);

    if($("#product-offer").is(":checked"))
	{
		price = parseFloat(navigatecms.forms.imask["product-offer_price"].unmaskedValue);
	}

	if($("#product-tax_class").val() == "custom")
	{
		price = price + ( (price / 100) * parseFloat(navigatecms.forms.imask["product-tax_value"].unmaskedValue));
	}
