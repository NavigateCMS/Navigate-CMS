...
function navigate_products_copy_from_theme_samples(element, section, language, type)
{
    var data = theme_content_samples;

    $("#navigate_products_copy_from_theme_samples_options").html("");
    ...
    for(i in data)
    {
		if(data[i].file)
		{
			$("#navigate_products_copy_from_theme_samples_options").append(
    ...
    $("#navigate_products_copy_from_theme_samples").dialog(
		{
			...
			[
				{
					...
					click: function()
					{
						if(type=="tinymce")
						{
							tinyMCE.get(element).execCommand('mceInsertContent', false, $("#navigate_products_copy_from_theme_samples_text_raw").html());
						}
						else // raw
						{
							$("#" + element).val($("#navigate_products_copy_from_theme_samples_text_raw").html() + " " + $("#" + element).html());
						}
			...
			open: function()
			{
				...
				navigate_products_copy_from_theme_samples_preview(
					$("#navigate_products_copy_from_theme_samples_options option:first").attr("value"),
					$("#navigate_products_copy_from_theme_samples_options option:first").attr("type"),
					$("#navigate_products_copy_from_theme_samples_options option:first").attr("source")
				);
			}
		}
    ).dialogExtend(
...
function navigate_products_copy_from_theme_samples_preview(value, type, source)
{
	if(source == "content")
	{
		$('#navigate_products_copy_from_theme_samples_text > *').remove();

		$('#navigate_products_copy_from_theme_samples_text').html(
			'<iframe width="100%" height="100" frameborder="0" src="about:blank"></iframe>'
		);

		$('#navigate_products_copy_from_theme_samples_text iframe').attr(
			'height',
			$('#navigate_products_copy_from_theme_samples_text').parent().parent().height() - 50
		);

		$("#navigate_products_copy_from_theme_samples_text iframe").on("load", function()
		{
			$(this).contents().find("body").html($.base64.decode(value));
		});

		// prepare content if the user wants to include it into the current editor
		$("#navigate_products_copy_from_theme_samples_text_raw").html(
	...
	else if(source == "file")
	{
		...
		$("#navigate_products_copy_from_theme_samples_text iframe").on("load", function()
		{
			$("#navigate_products_copy_from_theme_samples_text iframe")
				.contents()
				.find('img')
				.each(function()
				{
		...
		$.get(
			...
			function(data)
			{
				...
				$("#navigate_products_copy_from_theme_samples_text_raw").html( fragment_html ) ;

				$("#navigate_products_copy_from_theme_samples_text_raw img").each(function()
				{
					if( $(this).attr("src").substring(0, 7)!='http://'  &&
						$(this).attr("src").substring(0, 7)!='https://'
					  )
...
{
	var price = navigate_string_to_decimal($("#product-base_price").inputmask('unmaskedvalue'));

    if($("#product-offer").is(":checked"))
        price = navigate_string_to_decimal($("#product-offer_price").inputmask('unmaskedvalue'));

	if($("#product-tax_class").val() == "custom")
		price = price + ( (price / 100) * navigate_string_to_decimal($("#product-tax_value").inputmask('unmaskedvalue')));
