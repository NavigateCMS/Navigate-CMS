	...
	$("#navigate-properties-copy-from-dialog").dialog(
		{
			...
			open: function()
			{
				...
				$("#navigate_properties_copy_from_item_id").find(".buttonset").controlgroup('refresh');
                ...
                [
                    {
                        ...
                        click: function()
                        {
                            if(type=="tinymce") // or grapesjs
                            {
								if(navigate_editor_active(element) == "composer")
								{
									navigatecms.forms.grapesjs[element+"-composer"].getSelected().append($("#navigate_properties_copy_from_theme_samples_text_raw").html());
								}
								else // tinymce
								{
                    ...
                    },
                    {
                        ...
                        click: function()
                        {
                            if(type=="tinymce") // or grapesjs
                            {
                            	if(navigate_editor_active(element) == "composer")
								{
									navigatecms.forms.grapesjs[element+"-composer"].addComponents($("#navigate_properties_copy_from_theme_samples_text_raw").html());
								}
                            	else // tinymce
								{
...
function navigate_property_webuser(property_id)
{
	$("#"+property_id).select2(
	{
		placeholder: $("#"+property_id+"-helper").text(),
		minimumInputLength: 1,
		ajax: {
			url: "?fid=comments&act=json_find_webuser",
			dataType: "json",
			delay: 100,
...
function properties_run()
{
	...
	$("div[nv_property_type=webuser]").each(function()
	{
		navigate_property_webuser("property-" + $(this).attr("nv_property"));
	})
