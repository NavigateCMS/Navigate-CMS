...
$("#item-author-text").autocomplete(
    {
        source: function(request, response)
        {
            ...
            $.ajax(
                {
                    url: NAVIGATE_APP + "?fid=items&act=json_find_user&format=autocomplete",
                    dataType: "json",
                    method: "GET",
                    data: toFind,
...
function navigate_periodic_event_delegate()
{
	...
	{					
		for(s in template_sections)
		{
			...
			{
				case "tinymce":
					if(!tinyMCE.get(section_id) || tinyMCE.get(section_id).initialized != true)
					{
						continue;
					}

					if($('span.editor_selector[for="'+section_id+'"').find('i.active').data("action")=="composer")
					{
						navigate_editor_grapesjs_to_tinymce(section_id);
					}
...
{			
	...
	switch($("input[name=\'navigate_items_copy_from_type[]\']:checked").val())
	{
		case "language":
			...
			if(navigate_editor_active("section-" + copy_from_section + "-" + lang)=="composer")
			{
				navigate_editor_grapesjs_to_tinymce("section-" + copy_from_section + "-" + lang);
			}
			...
			navigate_editor_tinymce_to_grapesjs(dest);
			break;
					...
					},
					{
						...
						click: function()
						{
							if(type=="tinymce")
							{
								...
								navigate_editor_tinymce_to_grapesjs(element);
							}
							else // raw
							{
								$("#" + element).val(
    ...
    $("#navigate_items_copy_from").dialog(
        {
            ...
            open: function()
            {
				...
				$("#navigate_items_copy_from").find(".buttonset").controlgroup('refresh');
            }
        }
    );
}
...
$("#items-gallery-elements").on("contextmenu", "li:not(.gallery-item-empty-droppable)", function(e)
	{
		...
		setTimeout(function()
		{
			...
			$("#contextmenu-gallery-items-description").off("click").on("click", function()
			{
				...
				var file_id = $('#contextmenu-gallery-items').data("file-id");

				// try to retrieve the info from Gallery tab
				if($('#items-gallery-item-' + file_id + '-titles').length > 0)
				{
					titles = $('#items-gallery-item-' + file_id + '-titles').val();
					titles = Base64.decode(titles);
				...
				if($('#items-gallery-item-' + file_id + '-descriptions').length > 0)
				{
					descriptions = $('#items-gallery-item-' + file_id + '-descriptions').val();
					descriptions = Base64.decode(descriptions);
					descriptions = $.parseJSON(descriptions);
				}

				navigate_contextmenu_description_dialog($('#contextmenu-gallery-items').data("file-id"), trigger, titles, descriptions);
			});
...
{	
	...
	if($(el).attr("id")=="items-gallery-item-empty-droppable")
	{
		...
		var access_icons = clone.find(".file-access-icons").clone();
		...
		if(img.length > 0)
		{
			final_html = $("<div />").append(access_icons).append(img.clone()).html();
		}
		else
		{
			final_html = $("<div />").append(access_icons).append(clone.html()).html();
		}
		
		new_node = '<li>' +
				   '<div id="items-gallery-item-' + file_id + '-droppable" website_id="' + navigate_media_browser_website + '" data-file-id="' + file_id + '" class="navigate-droppable ui-corner-all" data-media="image">' +
				   final_html +
				   '</div> ' +
	...
	else
	{
		...
		if(img.length > 0)
		{
			$(el).html($("<div />").append(access_icons).append(img.clone()).html());
		}
		else
		{
			$(el).html($("<div />").append(access_icons).append(clone.html()).html());
		}
		$(el).attr("data-media", "image");
	}
	navigate_items_gallery_parse();
}
...
$("#object-comments-toolbar-publish").on("click", function()
{
	...
	$.ajax(
	{
		url: NAVIGATE_APP + "?fid=items&act=change_comment_status&id=" + ica + "&opt=publish",
		dataType: "text",
		method: "GET",
		success: function( data ) 
		{
			if(data=="true")	
				$("#object-comment-" + ica).attr("class", "object-comment-message object-comment-status-public");
		}
	});					
});
...
$("#object-comments-toolbar-unpublish").on("click", function()
{
	...
	$.ajax(
	{
		url: NAVIGATE_APP + "?fid=items&act=change_comment_status&id=" + ica + "&opt=unpublish",
		dataType: "text",
		method: "GET",
		success: function( data ) 
		{
			if(data=="true")
				$("#object-comment-" + ica).attr("class", "object-comment-message object-comment-status-private");
		}
	});					
});			
...
$("#object-comments-toolbar-delete").on("click", function()
{
	...
	navigate_confirmation_dialog(
		function()
		{
			$.ajax(
				{
					url: NAVIGATE_APP + "?fid=items&act=change_comment_status&id=" + ica + "&opt=delete",
					dataType: "text",
					method: "GET",
					success: function( data )
					{
						if(data=="true")
						{
							$("#object-comment-" + ica).parent().remove();
...
$(".object-comment-message").on("mousemove", function() { navigate_items_comment_toolbar(this); });

var item_comment_active = 0;
function navigate_items_comment_toolbar(el)
{
	item_comment_active = $(el).attr("id").substr("object-comment-".length);
...
if($("#navigate-panel-web-votes-graph").length > 0)
{
	...
	if(votes_graph_data_points.length > 0)
	{
		var plot = $.plot(
...
function navigate_items_tabform_submit(formNum)
{
	...
	// transfer all active grapesjs contents as tinymce contents (that is really what is saved)
	navigate_editor_selector_before_submit();

	var url = $('#navigate-content').find('form').eq(formNum).attr('action');
	$('#navigate-content').find('form').eq(formNum).attr(
		'action',
		$.query.load(url).set('tab', tab).set('tab_language', navigate_items_language_selected).toString()
	);
