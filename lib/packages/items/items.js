					...
					success: function( data ) 
					{
						...
						$.each(data, function(row) 
						{
							$("#navigate_items_copy_from_section").append(
                                $("<option />").val(data[row].code).html(data[row].title)
                            );
						});
...
{			
	...
	switch($("input[name=\'navigate_items_copy_from_type[]\']:checked").val())
	{
		case "language":
			...
			if(copy_from_section == "") copy_from_section = "main";
...
$("#embedding_info").qtip(
{
	content:    "<div><strong>" + $("#embedding_info").data("message-title-1") + "</strong>: " +
					$("#embedding_info").data("message-content-1") +
					"<br /><br /><strong>"  + $("#embedding_info").data("message-title-2") + "</strong>: " +
					$("#embedding_info").data("message-content-2") +
				"</div>",
