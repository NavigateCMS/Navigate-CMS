...
function navigate_periodic_event_delegate()
{
	...
	{					
		for(s in template_sections)
		{
			...
			{
				case "tinymce":
					if(tinyMCE.get(section_id).plugins.spellchecker.active == 1) continue;
...
// script#5
function navigate_items_path_check(el)
{	
	if($(el).val()=="") return;
...
{			
	switch($("input[name=\'navigate_items_copy_from_type[]\']:checked").val())
	{
		case "language":
			...
			tinyMCE.get(dest).setContent(tinyMCE.get("section-" + $("#navigate_items_copy_from_section").val() + "-" + lang).getContent({ format: "raw" }), 
										 { format: "raw" });		
			break;
...
{
    $.get(NAVIGATE_APP + "?fid=" + navigate_query_parameter('fid') + "&act=92&history=true&id="+id,
    function(data)
    {
        if(type=='tinymce')
        {
            $("#navigate_items_copy_from_history_text").html(data);
            $("#navigate_items_copy_from_history_text_raw").html(data);
        }
        else
        {
            $("#navigate_items_copy_from_history_text").html(phpjs_nl2br(data, false));
            $("#navigate_items_copy_from_history_text_raw").html(data);
        }
    });
...
function navigate_items_copy_from_theme_samples(element, section, language, type)
{
    ...
    for(i in data)
    {
        $("#navigate_items_copy_from_theme_samples_options").append('<option value="'+data[i].file+'" type="'+type+'">'+data[i].title+'</option>');
    }
                ...
                [
                    ...
                    },
                    {
                        ...
                        click: function()
                        {
                            if(type=="tinymce")
                            {
                                tinyMCE.get(element).setContent($("#navigate_items_copy_from_theme_samples_text_raw").html() + " " + tinyMCE.get(element).getContent(), { format: "raw" });
                            }
                            else // raw
                            {
...
function navigate_items_copy_from_theme_samples_preview(file, type)
{
    ...
    $('#navigate_items_copy_from_theme_samples_text').html('<iframe width="100%" height="100" frameborder="0" src="' + NAVIGATE_URL + "/themes/" + website_theme + "/" + file + '"></iframe>');
    $('#navigate_items_copy_from_theme_samples_text iframe').attr('height', $('#navigate_items_copy_from_theme_samples_text').parent().parent().height() - 50);

    $.get(
        NAVIGATE_URL + "/themes/" + website_theme + "/" + file,
        function(data)
        {
            $("#navigate_items_copy_from_theme_samples_text_raw").html($(data).filter('#navigate-theme-content-sample').html());
        }
    )
}
...
$("#item-comments_moderator-text").autocomplete(
{
	source: function(request, response)
	{
		var toFind = {	
	...
	select: function(event, ui) 
	{
		$("#item-comments_moderator").val(ui.item.id);
	}
});
...
$("#items-comments-toolbar-publish").bind("click", function()
{
...
$("#items-comments-toolbar-unpublish").bind("click", function()
{
	var ica = item_comment_active;
	$.ajax(
	{
...
$("#items-comments-toolbar-delete").bind("click", function()
{
	var ica = item_comment_active;
	$.ajax(
	{
...
$("#items-comments-toolbar").parent().bind("scroll", function()	 { $("#items-comments-toolbar").hide(); });
$(".items-comment-message").bind("mousemove", function() { navigate_items_comment_toolbar(this); });
...
{		
	...
	if($(el).position().top > 40)
		$("#items-comments-toolbar").css($(el).position()).show();
	else
		$("#items-comments-toolbar").hide();
