    ...
    if(!empty($comments[0]))
    {
        ...
        for($c=0; $c < $comments_limit; $c++)
        {
            ...
            $tmp = array(
                ...
                '</a>'.
                '</div>',
                '<div id="items-comment-'.$comments[$c]->id.'" class="navigate-panel-recent-comments-element">'.core_special_chars($comments[$c]->message).'</div>');
...
function dashboard_panel_latest_searches($params)
{
    ...
    if(!empty($data))
    {
        for($e = 0; $e < 5; $e++)
        {
            ...
            $data_html .= '
                <div class="navigate-panel-recent-comments-username ui-corner-all items-comment-status-public">'.
                '<div>'.core_ts2date($data[$e]['date']).' <img align="absmiddle" src="img/icons/silk/bullet_star.png" align="absmiddle"> '.core_special_chars($data[$e]['text']).'</div>'.
                '</div>';
        }
