...
function nvweb_list($vars=array())
{
    ...
    else if($vars['source']=='order')
    {
        $order = new order();
        $order->load($vars['id']);
        $order->load_lines();
        $rs = $order->lines;
        $total = count($order->lines);
    }
	...
	switch($nvlist_tag_source)
	{
        ...
        case 'order':
            ...
            switch($tag['attributes']['value'])
            {
                ...
                case 'quantity':
                    $out = core_decimal2string($item->quantity);
                    break;

                case 'price':
                    $base_price_wtax = $item->base_price + $item->base_price_tax_amount;
                    $out = core_price2string($base_price_wtax, $item->currency, @$tag['attributes']['return']);
                    break;

                case 'coupon_amount':
                    if(!empty($item->coupon_amount))
                    {
                        $out = core_price2string($item->coupon_amount, $item->currency, @$tag['attributes']['return']);
                    }
                    break;

                case 'coupon_unit':
                    if(!empty($item->coupon_unit))
                    {
                        $out = core_price2string($item->coupon_unit, $item->currency, @$tag['attributes']['return']);
                    }
                    break;

                case 'subtotal':
                    $out = core_price2string($item->total, $item->currency, @$tag['attributes']['return']);
                    break;

                case 'subtotal_with_taxes_without_coupon':
                    $out = core_price2string($item->total + $item->coupon_amount, $item->currency, @$tag['attributes']['return']);
                    break;
